...
class SqrtOpLowering(RewritePattern, VmCallOpImpl):
    @op_rewrite_pattern
    def match_and_rewrite(self, op: math.SqrtOp, rewriter: PatternRewriter) -> bool:
        return self.convert(op, rewriter)


class ConvertMathToVmPass(Pass):
