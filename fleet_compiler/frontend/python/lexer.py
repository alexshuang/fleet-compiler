...
class Tokenizer:
    ...
    def get_token(self):
        ...
        if is_alpha(ch):
            ...
            self.raise_error(SyntaxErrorCode.Token,
                             f"Unrecognized token which start with {ch}")
    ...
    def parse_operator(self):
        ...
        if data not in op_str2code:
            self.raise_error(SyntaxErrorCode.Operator, f"Unsupported operator {data} here")
    ...
    def parse_string_literal(self):
        ...
        if self.stream.eof():
            self.raise_error(SyntaxErrorCode.StringLiteral, f"Should end with {quote} here")
    ...
    def parse_identifier(self):
        ...
        while not self.stream.eof() and not (is_separator(ch) or is_operator(ch) or is_terminator(ch)):
            if not is_valid(ch):
                self.raise_error(SyntaxErrorCode.Identifier, f"Invalid identifier {data} with char {ch}")
            data += ch
            self.stream.next()
            ch = self.stream.peak()
    ...
    def parse_digit(self):
        ...
        while not self.stream.eof() and not (is_separator(ch) or is_operator(ch) or is_terminator(ch)):
            if not is_valid(ch):
                self.raise_decimal_error(f"Invalid number {data}")
            data += ch
            if ch == '.':
                if dot_cnt > 0:
                    self.raise_decimal_error(f"Redundant decimal point {data}")
                dot_cnt += 1
            self.stream.next()
            ch = self.stream.peak()
    ...
    def skip_comments(self):
        ...
        while not self.stream.eof():
            if self.stream.peak() == '\'' and ch1 == ch2 == '\'':
                ...
                ch1 = ch2
                ch2 = self.stream.next()
        self.raise_error(SyntaxErrorCode.Comment, f"Should end with ''' here")
    ...
    def raise_error(self, code, msg):
        raise SyntaxException(code, f"{self.stream.location_str} : {msg}")
    
    def raise_decimal_error(self, msg):
        self.raise_error(SyntaxErrorCode.DecimalLiteral, msg)
